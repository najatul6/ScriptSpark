...
const LinkToScript = () => {
  ...
  const handleExtractScript = async () => {
    ...
    try {
      ...
      toast.error(
        "AI could not access this link directly. Try pasting the script manually.",
      );
    } finally {
      ...
      <main className="w-full max-w-3xl space-y-6">
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-2xl shadow-xl">
          <label className="flex items-center gap-2 text-sm font-medium text-slate-300 mb-3">
            <Briefcase size={18} className="text-blue-400" /> Select Your
            Profession
          </label>
          <select
            value={profession}
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-2xl shadow-xl">
          <label className="flex items-center gap-2 text-sm font-medium text-slate-300 mb-3">
            <FileText size={18} className="text-purple-400" /> Analyzed Script /
            Summary
          </label>
          <textarea
          onClick={handleClick}
          ...
          <div className="bg-slate-900/90 border border-purple-500/40 p-6 rounded-2xl animate__animated animate__fadeInUp">
            <div className="flex justify-between mb-3 text-purple-400 font-bold uppercase text-xs tracking-widest">
              ...
              <button
                ...
                className="hover:text-white transition-colors flex items-center gap-1 text-[10px] bg-white/5 px-2 py-1 rounded"
              >
                <Copy size={14} /> COPY
              </button>
            </div>
